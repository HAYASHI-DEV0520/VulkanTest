...
private:
	...
	std::vector<VkImage> swapChainImages; // no need to clean
	...
	VkCommandPool commandPoolGraphic;
	VkCommandPool commandPoolTransfer;
	std::vector<VkCommandBuffer> commandBuffersGraphic; // dont need to destroy
	VkCommandBuffer commandBuffersTransfer;
	...
	*/
	...
	void initWindow() {
		...
		inputManager.registerEvent(GLFW_MOUSE_BUTTON_MIDDLE, [&](InputManager* input) {cameraManager.moveByKey(input, GLFW_MOUSE_BUTTON_MIDDLE); });
	...
	*/
...
#ifndef NDEBUG
			std::cout << "FINAL SCORE: " << candidates.rbegin()->first << std::endl;
#endif
		}
		else {
			throw std::runtime_error("failed to find a suitable GPU!");
	...
	void createSwapChain() {
		...
		createInfo.oldSwapchain = VK_NULL_HANDLE; // affect when window is resized
	...
	VkPresentModeKHR chooseSwapPresentMode(const std::vector<VkPresentModeKHR>& availablePresentModes) {
		for (const auto& availablePresentMode : availablePresentModes) {
			if (availablePresentMode == VK_PRESENT_MODE_MAILBOX_KHR) {				// only use it when energy usage is not a concern
	...
	void createRenderPass() {
		...
		colorAttachment.loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR; // clear display always
		colorAttachment.storeOp = VK_ATTACHMENT_STORE_OP_STORE; // to display
		colorAttachment.stencilLoadOp = VK_ATTACHMENT_LOAD_OP_DONT_CARE;
		colorAttachment.stencilStoreOp = VK_ATTACHMENT_STORE_OP_DONT_CARE;
		colorAttachment.initialLayout = VK_IMAGE_LAYOUT_UNDEFINED; // dont care previous layout
		colorAttachment.finalLayout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL; // no longer ->(operated when rendering end, swap chain start)

		VkAttachmentReference colorAttachmentRef{};
		colorAttachmentRef.attachment = 0;
		colorAttachmentRef.layout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL; // for best performance
		...
		subpass.pResolveAttachments = &colorAttachmentResolveRef; // place where multisample resolve operation defined
	...
	void createGraphicsPipeline() {
		...
		// 
		// disabled for DynamicStates creation

		VkPipelineRasterizationStateCreateInfo rasterizer{};
		rasterizer.sType = VK_STRUCTURE_TYPE_PIPELINE_RASTERIZATION_STATE_CREATE_INFO;
		rasterizer.depthClampEnable = VK_FALSE; // fragment clamping disabled
		rasterizer.rasterizerDiscardEnable = VK_FALSE;
		rasterizer.polygonMode = VK_POLYGON_MODE_FILL;
		rasterizer.lineWidth = 1.0f;
		rasterizer.cullMode = VK_CULL_MODE_BACK_BIT;
		rasterizer.frontFace = VK_FRONT_FACE_COUNTER_CLOCKWISE;
		rasterizer.depthBiasEnable = VK_FALSE; // used for shadow-mapping sometimes
		...
		depthStencil.depthCompareOp = VK_COMPARE_OP_LESS; // render closer items
		depthStencil.depthBoundsTestEnable = VK_FALSE; // allows to only keep frags that fall within
		...
		*/
		...
		pipelineInfo.subpass = 0; // index of subpass array
	...
	void createCommandPool() {
		...
		VkCommandPoolCreateInfo poolInfoGraphic{};
		poolInfoGraphic.sType = VK_STRUCTURE_TYPE_COMMAND_POOL_CREATE_INFO;
		poolInfoGraphic.flags = VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT;
		poolInfoGraphic.queueFamilyIndex = queueFamilyIndices.graphicsFamily.value();

		if (vkCreateCommandPool(device, &poolInfoGraphic, nullptr, &commandPoolGraphic) != VK_SUCCESS) {
	...
	void createVertexBuffer() {
		...
		vertexBufferMemory.resize(meshes.size());
		for (int i = 0; i < meshes.size(); i++ ) {
			VkDeviceSize bufferSize = sizeof(Vertex) * meshes[i].vertices.size();
			...
			createBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
				VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, vertexBuffers[i], vertexBufferMemory[i], true);

			copyBuffer(stagingBuffer, vertexBuffers[i], bufferSize, 0);
	...
	void copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size, size_t differenciate) {
		VkCommandBuffer commandBuffer = setupCommandBuffer(std::nullopt, true);
		...
		flushCommandBuffer(commandBuffer, differenciate, true);
	}


	VkCommandBuffer setupCommandBuffer(std::optional<VkCommandBuffer> commandBufferIn, bool isTransfer) {
	...
	void flushCommandBuffer(VkCommandBuffer commandBuffer, size_t differenciate, bool isTransfer) {
	...
	void loadModel() {
		...
		loadTexturesFromMaterials(materials, TEXTURE_PATH);
	}

	void createIndexBuffer() {
		indexBuffers.resize(meshes.size());
		indexBufferMemory.resize(meshes.size());
		for (int i = 0; i < meshes.size(); i++) {
			VkDeviceSize bufferSize = sizeof(uint32_t) * meshes[i].indices.size();
			...
			createBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
				indexBuffers[i], indexBufferMemory[i], true);

			copyBuffer(stagingBuffer, indexBuffers[i], bufferSize, 0);
	...
	void createUniformBuffers() {
		...
		for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
			createBuffer(bufferSize, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
				...
				VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, uniformBuffers[i], uniformBuffersMemory[i], true);
	...
	void createDescriptorPool() {
		...
		poolSize[0].descriptorCount = static_cast<uint32_t>(MAX_FRAMES_IN_FLIGHT * meshes.size());

		poolSize[1].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
		poolSize[1].descriptorCount = static_cast<uint32_t>(MAX_FRAMES_IN_FLIGHT * meshes.size());
	...
	void createDescriptorSets() {
		descriptorSets.assign(MAX_FRAMES_IN_FLIGHT, std::vector<VkDescriptorSet>(meshes.size()));

		std::vector<VkDescriptorSetLayout> layouts(MAX_FRAMES_IN_FLIGHT * meshes.size(), descriptorSetLayout);
		...
		for (int f = 0; f < MAX_FRAMES_IN_FLIGHT; f++) {
			for (int m = 0; m < meshes.size(); m++) {
				descriptorSets[f][m] = allSets[idx++];
			}
		}
		...
		for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
			for (int m = 0; m < meshes.size(); m++) {
				...
				imageInfo.imageView = textures[m].view;
				imageInfo.sampler = textures[m].sampler;

				std::array<VkWriteDescriptorSet, 2> descriptorWrite{};
				descriptorWrite[0].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
				descriptorWrite[0].dstSet = descriptorSets[i][m];
				...
				descriptorWrite[1].dstSet = descriptorSets[i][m];
	...
	void createCommandBuffers() {
		commandBuffersGraphic.resize(MAX_FRAMES_IN_FLIGHT);
		VkCommandBufferAllocateInfo allocInfo{};
		allocInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
		allocInfo.commandPool = commandPoolGraphic;
		allocInfo.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
		allocInfo.commandBufferCount = static_cast<uint32_t>(commandBuffersGraphic.size());

		if (vkAllocateCommandBuffers(device, &allocInfo, commandBuffersGraphic.data()) != VK_SUCCESS) {
	...
	Texture createTextureFromFile(const std::string& filename) {
		...
		*/
		...
		generateMipmaps(tex.image, VK_FORMAT_R8G8B8A8_SRGB, texWidth, texHeight, mipLevels, filename);
	...
	void loadTexturesFromMaterials(const std::vector<tinyobj::material_t>& materials,
		const std::string& basedir) {
		textures.clear();
		textures.resize(std::max(static_cast<int>(materials.size()), 1));
		size_t i = 0;
		for (; i < materials.size(); i++) {
			...
			if (!m.diffuse_texname.empty()) {
				...
				textures[i] = createTextureFromFile(TEXTURE_PATH_OPT);
			}
		}
		if (i == 0) {
			textures[0] = createTextureFromFile(TEXTURE_PATH_OPT);
		}
	}

	void generateMipmaps(VkImage image, VkFormat imageFormat, int32_t texWidth, int32_t texHeight, uint32_t mipLevels, const std::string& filename) {
		/*
	...
	VkImageView createImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags, uint32_t mipLevels) {
		...
		//left out the explicit viewInfo.components because VK_COMPONENT_SWIZZLE_IDENTITY is defined as 0
	...
	void createImage(uint32_t width, uint32_t height, uint32_t mipLevels, VkSampleCountFlagBits numSamples, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage,
		...
		imageInfo.tiling = tiling; //not linear because of the usage of staging buffer instead of staging image
	...
	void transitionImageLayout(VkCommandBuffer commandBuffer, VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, uint32_t mipLevels) {
		...
		barrier.srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
		barrier.dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED; // must set to this instead of default value
		barrier.image = image;
		...
		if (oldLayout == VK_IMAGE_LAYOUT_UNDEFINED && newLayout == VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL) {
			...
			sourceStage = VK_PIPELINE_STAGE_HOST_BIT; // pairing to host_write_bit
	...
	void createSyncObjects() {
		...
		for (size_t i = 0; i < TRANSFER_COUNT; i++) {
			if (vkCreateFence(device, &fenceInfo, nullptr, &transferFences[i]) != VK_SUCCESS) {
				throw std::runtime_error("failed to create transfer fences!");
			}
		}
	}
	...
	void createDepthResources() {
		...
		flushCommandBuffer(commandBuffer, 3, false);
	}
	...
	QueueFamilyIndices findQueueFamilies(VkPhysicalDevice device) {
		...
		for (const auto& queueFamily : queueFamilies) {
			...
			} else //use a independent queue for data tranform
	...
	void drawFrame() {
		...
		vkResetCommandBuffer(commandBuffersGraphic[currentFrame], 0);
		recordCommandBuffer(commandBuffersGraphic[currentFrame], imageIndex);
	...
	void cleanup() {
		...
		vkDestroyCommandPool(device, commandPoolGraphic, nullptr);
		...
		vkDestroySurfaceKHR(instance, surface, nullptr);// GLFW when creating, vk when destroying
